...
is_draft: true
author: gummibeer
description: 'How I use session transcripts, a deliberately strict prompt and Notion to turn our D&D sessions into concrete roleplay feedback and long-term trends.'
---

For our current D&D campaign I already had detailed session transcripts, summaries and old feedback.

At some point I started using all of that for something else: reviewing how we actually play.
...
This is a transcript of real people and the output is basically a performance review of a hobby session.

So before anything else: everyone involved has to be okay with the recording/transcription and with this kind of review. If somebody doesn't want an individual review, they don't get one.
...
For a session I can have multiple transcript sources. In the latest reviews, for example, MyArchivist and NoteCat were the useful diarized sources while a YouTube transcript was only good enough for rough orientation because it had no reliable speaker mapping.
...
That second part is what makes the whole thing useful beyond one evening.

I store the result in Notion.

At the time of writing, my two review databases contain **18 session review runs** and **107 player/DM review records**. Some sessions were reviewed by more than one model, which turned out to be especially useful.
...
The prompt currently has three awards:
...
The last one sounds harsher than it is supposed to be.

It doesn't mean "worst player". A strong player with one recurring habit that keeps flattening scenes can be a much better Growth Target than somebody who simply had a quiet session.
...
Every review ends with something like this:
...
For me it works because I already have the transcripts, the campaign data and Notion. The review turns data I already produce into concrete feedback I can actually use in the next session.

And the useful output is surprisingly small.

Sometimes it's a player realizing that almost all of their good ideas happen out of character.

Sometimes it's me noticing that I create a strong emotional beat and then immediately kill it with inventory discussion.

Sometimes it's the same criticism showing up three sessions in a row until there is really no point pretending it is a one-off anymore.
